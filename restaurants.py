...
def get_restaurants():
	...
	rad, err = get_num(request, 'radius', 1, 3, required=True)
	kwrd, err = utils.get_field(request, 'keyword', required=True)
	min_price, err = get_num(request, 'min_price', 0, 4, required=True)
	max_price, err = get_num(request, 'max_price', min_price, 4, required=True)
