...
get_friends_activity = 'SELECT first_name, last_name, photo_url, liked, res_name, review_date, user_ratings.user_id FROM user_ratings JOIN friends ON user_ratings.user_id = friends.user_id2 JOIN restaurants ON user_ratings.res_id = restaurants.res_id JOIN users on users.user_id = user_ratings.user_id WHERE friends.user_id1 = %s;'
